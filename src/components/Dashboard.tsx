...
import { BookOpen, Brain, Trophy, Layers, TrendingUp, Zap } from 'lucide-react';
import { modules } from '@/data/flashcards';

interface DashboardProps {
  onNavigate: (view: string, module?: string) => void;
  stats: { cardsReviewed: number; quizScore: number; streak: number };
}
          ...
          {modules.map((mod, i) => {
            ...
            return (
              <motion.button
                ...
                className="flex items-center gap-4 bg-card rounded-xl shadow-card p-4 hover:shadow-card-hover transition-shadow text-left"
              >
